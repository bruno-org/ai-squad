...
**Toda rodada roda o conjunto inteiro de casos.** Caso que passou não fica aposentado: ele volta a rodar na próxima rodada, e na seguinte, até o fim. E compare caso a caso contra a rodada anterior, nunca pelo total, porque total parado esconde a troca de um acerto por um erro.

**A razão é que comportamento de IA não tem fronteira limpa entre partes.** Diferente de teste de código, onde dá para saber que um arquivo não depende do outro, aqui todo caso lê o mesmo texto de instrução. Um ajuste pensado para consertar um caso muda o comportamento de outro caso que não tinha nada a ver, sem intenção e sem aviso. Rodar só o pedaço que falhou dá uma foto falsa: o caso que ficou de fora pode ter acabado de quebrar com a sua última correção, e você descobre isso com o produto no ar, na frente do cliente. **Onde economizar é no juiz, não na cobertura.**
